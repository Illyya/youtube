...
function plottingResults(items) {
  items.forEach((el) => {
    ...
    const formattedDate = `Дата: ${date.getFullYear()}-${addZero(date.getMonth() + 1)}-${addZero(date.getDate())} Время: ${addZero(date.getHours())}:${addZero(date.getMinutes())}:${addZero(date.getSeconds())}`
